...
if (!localStorage['lastTool']) localStorage['lastTool']='line';
function editor_obj()
	{
		var ry2 = 0;
		var r1 = 0;
...
//		tool[currentLevel.type].draw(inX.canvas,inX.ctx,currentLevel)
	...
	function changeTool(inTool)
		{
		if(inTool.type=='undo' && inTool.type=='redo' && inTool.type=='line') {}
		else
...
//	$(document.body).bind('keydown','alt+z',function ()
	...
	$('#submit').click(function ()
	{
		...
		$('#successmsg').fadeIn(100).show();

		setTimeout(function() {
			$('#successmsg').fadeOut('fast');
		}, 4000);
		document.getElementById('divCanvasData').style.display = "block";*/
	...
	$('#email').mousedown(function(evt){
		element.focus();
	});

	var element2 = $('#password')[0];
	
	$('#password').mousedown(function(evt){
		element2.focus();
	});

	$('#export').click(function(){
			window.setTimeout(function () {editor.createLastCanvas('export')},0)
			var substring = "kotter.net";
			url = "";
			chrome.windows.getAll({populate:true},function(windows){
				windows.forEach(function(window){
					window.tabs.forEach(function(tab){
						if(tab.url.includes(substring))
						{
							url = tab.url;
						}
					});
			...
			{
				document.getElementById('displaypopuplogout').style.display = "block";
	...
	$('#continue').click(function(){
		div_hide();
		editor.createLastCanvas('export');
...
//Finish binding
	...
	function div_hide(){
		testvar = false;
		document.getElementById('displaypopup').style.display = "none";
		document.getElementById('displaypopuplogout').style.display = "none"
		document.getElementById('divCanvasData').style.display = "block";
	}
	function hide_popup(){
		testvar = false;
	...
	function show_div(){
        ...
        document.getElementById('divCanvasData').style.display = "none";
	}
	...
	function checkempty() {
		...
		else
		{
			...
			if(url.includes("qa")){
				fullurl = "https://qa.onescreen.kotter.net/newlogin";
			}
			else if(url.includes("staging")){
				fullurl = "https://staging.onescreen.kotter.net/newlogin";
			}
			else if(url.includes("bridge")){
				fullurl = "https://onescreen.kotter.net/newlogin";
			}
			else
			{
				fullurl = "https://onescreen.kotter.net/newlogin";
	...
	function sendimagdata() {
		
		image64 = image64.slice(22);
		console.log(image64);
		token = localStorage.token;
		fullurl2 = "";

			if(url.includes("qa")){
				fullurl2 = "https://qa.onescreen.kotter.net/hawksoft/uploadscreenshot";
			}
			else if(url.includes("staging")){
				fullurl2 = "https://staging.onescreen.kotter.net/hawksoft/uploadscreenshot";
			}
			else if(url.includes("bridge")){
				fullurl2 = "https://onescreen.kotter.net/hawksoft/uploadscreenshot";
			}
			else{
				fullurl2 = "https://onescreen.kotter.net/hawksoft/uploadscreenshot";
			}
	...
	$('#topText').html
		...
		// Ready for action
		 ...
		 xhr.setRequestHeader('Authorization', 'OAuth ' + googleAuth.getAccessToken())
			...
			"mimeType": "image/png",
			"description": 'Taken by The Kotter Group. http://www.kotter.net/'
			};
