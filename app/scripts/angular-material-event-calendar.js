        ...
        $delegate.throttle = function(cb) {
            ...
            return function debounced() {
                ...
                if (!alreadyQueued) {
                    alreadyQueued = true;
                    $delegate(function() {
                        queueCb.apply(context, Array.prototype.slice.call(
                            queuedArgs));
                        alreadyQueued = false;
                    });
                }
...
(function() {
    "use strict";
    angular.module("material.components.eventCalendar").run(["$templateCache", function(
        $templateCache) {
        $templateCache.put("icons/ic_close_black_24px.svg",
            "<svg fill=\"#000000\" height=\"24\" viewBox=\"0 0 24 24\" width=\"24\" xmlns=\"http://www.w3.org/2000/svg\">\n    <path d=\"M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z\"/>\n    <path d=\"M0 0h24v24H0z\" fill=\"none\"/>\n</svg>"
        );
        $templateCache.put("icons/ic_keyboard_arrow_right_black_24px.svg",
...
(function() {
    "use strict";
    eventCalendarDirective.$inject = ["$injector", "$parse"];
    angular
        .module('material.components.eventCalendar')
    ...
    function eventCalendarDirective($injector, $parse) {
        ...
        function compile(tElement, tAttr) {
            var eventClickFunc = tAttr.mdEventClick ? $parse(tAttr.mdEventClick, null, true) :
                undefined;
            var createEventClickFunc = tAttr.mdCreateEventClick ? $parse(tAttr.mdCreateEventClick,
                null, true) : undefined;
            var mdCreateDisabled = tAttr.mdCreateDisabled ? $parse(tAttr.mdCreateDisabled) :
                undefined;
            ...
            return function postLink(scope, element, attrs, ctrls) {
                ...
                function callEventClick(e, eventItem) {
                    if (!attrs.mdEventClick) {
                        return;
                    }
                    eventClickFunc(scope.$parent, {
                        $event: e,
                ...
                function createEventClick(e, date) {
                    if (!attrs.mdCreateEventClick) {
                        return;
                    }
                    createEventClickFunc(scope.$parent, {
                        $event: e,
                        $date: date
                    });
                }
                ...
                if (mdCreateDisabled) {
                    scope.$watch(function() {
                        return mdCreateDisabled(scope.$parent);
                    }, function(value) {
        ...
        function controller($$mdEventCalendarUtil, $element, $attrs) {
            ...
            vm.showCreateLink = $attrs.mdShowCreateLink !== undefined && $attrs.mdShowCreateLink !==
                'false';
            ...
            vm.offset = vm.autoHeight === false || $attrs.autoHeight === '' || isNaN($attrs.autoHeight
                .replace('px', '')) ? 0 : parseInt($attrs.autoHeight.replace('px', ''));
...
(function() {
    "use strict";
    mdEventCalendarBuilderService.$inject = ["$$mdEventCalendarUtil", "$templateCache"];
    angular
        .module('material.components.eventCalendar')
    ...
    function mdEventCalendarBuilderService($$mdEventCalendarUtil, $templateCache) {
        ...
        function showMore(opts) {
            ...
            events.forEach(function(item) {
                ...
                var isEndThisDay = $$mdEventCalendarUtil.isValidDate(item.end) ?
                    $$mdEventCalendarUtil.isSameDay(date, item.end) : true;
                var eventOptions = {
        ...
        function filterEventsOnDay(date, events) {
            return !events || !events.length ? [] : events.filter(function(item) {
                return $$mdEventCalendarUtil.isDateWithinRange(date, item.start, item.end ||
                    item.start);
            }).sort(function(a, b) {
                a = new Date(a.start);
                b = new Date(b.start);
        ...
        function month(options) {
            ...
            var nextdate;
            nextdate = $$mdEventCalendarUtil.getDateInNextMonth(date);
            while (row.childNodes.length < 7) {
                if (dayOfWeek === 6) {
                    lastCalendarDay = true;
                }
                row.appendChild(createCellElement(getCellOptions(nextdate, dayOfWeek, true)));
        ...
        function createCellElement(options) {
            ...
            if (options.differentMonth === true) {
                cell.classList.add('different-month');
            }
            if ($$mdEventCalendarUtil.isSameDay(options.date, options.today)) {
            ...
            matchingEvents.every(function(eventItem, pos) {
                ...
                while (i < placeDiff) {
                    if (place >= options.maxEvents) {
                        cellContent.appendChild(createShowMore(matchingEvents.length -
                            pos, options.date));
                        return false;
                    }
                ...
                if (place >= options.maxEvents) {
                    cellContent.appendChild(createShowMore(matchingEvents.length - pos,
                        options.date));
                    return false;
                }
        ...
        function createEventElement(type, eventItem, options) {
            ...
            if (eventItem.customClass) {
                eventElement.classList.add(eventItem.customClass);
            }
            ...
            if (type.hasLabel === true) {
                ...
                if (type.allDay !== true) {
                    ...
                    dateLabelTime.textContent = $$mdEventCalendarUtil.formatEventTime(eventItem
                        .start);
        ...
        function getEventDisplayType(item, options) {
            ...
            var isEndThisDay = $$mdEventCalendarUtil.isValidDate(item.end) ?
                $$mdEventCalendarUtil.isSameDay(options.date, item.end) : true;
        ...
        function filterCurrentCalendar(date, events) {
            ...
            return events.filter(function(item) {
                if (!$$mdEventCalendarUtil.isValidDate(item.start)) {
                    return false;
                }
                if ($$mdEventCalendarUtil.isDateWithinRange(item.start, start, end)) {
                    return true;
                }
                if (!$$mdEventCalendarUtil.isValidDate(item.end)) {
                    return false;
                }
                if ($$mdEventCalendarUtil.isDateWithinRange(item.end, start, end)) {
                    return true;
                }
                return false;
            }).sort(function(a, b) {
...
(function() {
    "use strict";
    eventCalendarMonthDirective.$inject = ["$$mdEventCalendarBuilder", "$window", "$$rAF",
        "$timeout"
    ];
    angular
        .module('material.components.eventCalendar')
    ...
    function eventCalendarMonthDirective($$mdEventCalendarBuilder, $window, $$rAF, $timeout) {
        ...
        function link(scope, element, attrs, ctrl) {
            ...
            scope.$watch(function() {
                return mdEventCalendarCtrl.date;
            }, buildView);
            scope.$watch(function() {
                return mdEventCalendarCtrl.events;
            ...
            scope.$watch(function() {
                return mdEventCalendarCtrl.selectedEvents;
            }, function(newValue, oldValue) {
                if (newValue === oldValue) {
                    return;
                }
            ...
            function setAutoHeight() {
                if (!mdEventCalendarCtrl.autoHeight) {
                    return;
                }
            ...
            element.on('click', function(e) {
                ...
                if (eventId) {
                    ...
                    scope.$apply(function() {
                        mdEventCalendarCtrl.selectEvent(e, getIdFromHash(
                            eventId));
                    });
                    return;
                }
            ...
            function positonShowMore() {
                ...
                if (leftDiff > 0) {
                    ...
                    leftDiff -= 10;
                    if (leftDiff > 0) {
                        showMoreData.element.querySelector(
...
(function() {
    "use strict";
    utilService.$inject = ["$injector", "$locale", "$filter"];
    angular
        .module('material.components.eventCalendar')
    ...
    function utilService($injector, $locale, $filter) {
        var $mdDateLocale = $injector.has('$mdDateLocale') ? $injector.get('$mdDateLocale') :
            undefined;
        ...
        var shortDays = $mdDateLocale ? $mdDateLocale.shortDays : $locale.DATETIME_FORMATS.SHORTDAY
            .map(function(day) {
