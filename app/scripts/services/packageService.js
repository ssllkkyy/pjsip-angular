...
proySymphony.service('packageService', function(dataFactory, $filter, usefulTools, $rootScope) {
    ...
    service.init = function() {
        service.getInfoForPackages();
    };

    service.getInfoForPackages = function() {
        dataFactory.getInfoForPackages()
    ...
    service.getRequiredPackage = function(option, current) {
        if (current.package_options.indexOf(option) !== -1) return current.package_title;
        var current = service.getNextPackageByLevel(current.level);
        if (!current) return null;
        return service.getRequiredPackage(option, current);
    };

    service.getEnabledPackages = function() {
        var packages = $filter('filter')(service.availPackages, {
            'enabled': true
        });
        return packages;
    };
    ...
    service.getNextPackageByLevel = function(level) {
        var i, input = service.getEnabledPackages();
        for (i = 0; i < input.length; i += 1) {
