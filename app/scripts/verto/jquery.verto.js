...
(function($) {
    ...
    var generateGUID = (typeof(window.crypto) !== 'undefined' && typeof(window.crypto.getRandomValues) !==
            'undefined') ?
        function() {
            ...
            return (S4(buf[0]) + S4(buf[1]) + "-" + S4(buf[2]) + "-" + S4(buf[3]) + "-" + S4(
                buf[4]) + "-" + S4(buf[5]) + S4(buf[6]) + S4(buf[7]));
        }
    ...
    $.verto = function(options, callbacks) {
        ...
        if (verto.options.deviceParams.useCamera) {
            $.FSRTC.getValidRes(verto.options.deviceParams.useCamera, verto.options.deviceParams
                .onResCheck);
        }
    ...
    $.verto.prototype.deviceParams = function(obj) {
        ...
        if (obj.useCamera) {
            $.FSRTC.getValidRes(verto.options.deviceParams.useCamera, obj ? obj.onResCheck :
                undefined);
        }
    };
    ...
    $.verto.prototype.message = function(msg) {
        ...
        if (!msg.to) {
            if (__env.enableDebug) console.error("Missing To");
            err++;
        }

        if (!msg.body) {
            if (__env.enableDebug) console.error("Missing Body");
            err++;
        }
    ...
    function drop_bad(verto, channel) {
        if (__env.enableDebug) console.error("drop unauthorized channel: " + channel);
    ...
    function mark_ready(verto, channel) {
        for (var j in verto.eventSUBS[channel]) {
            ...
            if (__env.enableDebug) console.log("subscribed to channel: " + channel);
    ...
    $.verto.prototype.unsubscribe = function(handle) {
        ...
        if (!handle) {
            ...
            for (var u in unsubChannels) {
                if (__env.enableDebug) console.log("Sending Unsubscribe for: ", u);
                sendChannels.push(u);
            }
    ...
    $.verto.prototype.purge = function(callID) {
        ...
        for (i in verto.dialogs) {
            if (!x) {
                if (__env.enableDebug) console.log("purging dialogs");
            }
            x++;
        ...
        for (i in verto.eventSUBS) {
            if (verto.eventSUBS[i]) {
                if (__env.enableDebug) console.log("purging subscription: " + i);
    ...
    $.verto.prototype.newCall = function(args, callbacks) {
        ...
        if (!verto.rpcClient.socketReady()) {
            if (__env.enableDebug) console.error("Not Connected...");
            return;
        }
    ...
    $.verto.prototype.handleMessage = function(data) {
        ...
        if (!(data && data.method)) {
            if (__env.enableDebug) console.error("Invalid Data", data);
            return;
        }
        ...
        if (data.params.callID) {
            ...
            if (dialog) {
                ...
                switch (data.method) {
                    ...
                    case 'verto.info':
                        ...
                        break;
                    default:
                        if (__env.enableDebug) console.debug(
                ...
                switch (data.method) {
                    case 'verto.attach':
                        ...
                        dialog = new $.verto.dialog($.verto.enum.direction.inbound, verto,
                            data.params);
            ...
            switch (data.method) {
                ...
                case 'verto.event':
                    ...
                    if (!list && key && key === verto.sessid) {
                        if (verto.callbacks.onMessage) {
                            verto.callbacks.onMessage(verto, null, $.verto.enum.message.pvtEvent,
                                data.params);
                        }
                        ...
                        if (__env.enableDebug) console.error("UNSUBBED or invalid EVENT " +
                            key + " IGNORED");
                    } else {
                        for (var i in list) {
                            var sub = list[i];
                            ...
                            if (!sub || !sub.ready) {
                                if (__env.enableDebug) console.error("invalid EVENT for " +
                                    key + " IGNORED");
                            } else if (sub.handler) {
                                ...
                                if (__env.enableDebug) console.log("EVENT:", data.params);
                            }
                        }
                    }
                ...
                case "verto.info":
                    if (verto.callbacks.onMessage) {
                        verto.callbacks.onMessage(verto, null, $.verto.enum.message.info,
                            data.params.msg);
                    }
                    //console.error(data);
                    if (__env.enableDebug) console.debug("MESSAGE from: " + data.params.msg
                        .from, data.params.msg.body);

                    break;

                default:
                    if (__env.enableDebug) console.error(
                        "INVALID METHOD OR NON-EXISTANT CALL REFERENCE IGNORED", data.method
                    );
                    break;
            }
        }
    ...
    var hashArray = function() {
        ...
        vha.del = function(name) {
            ...
            if (hash[name]) {
                ...
                if (__env.enableDebug) console.error("can't del nonexistant key " +
                    name);
            }
    ...
    $.verto.liveArray = function(verto, context, name, config) {
        ...
        la.checkSerno = function(serno) {
            ...
            if (lastSerno > 0 && serno != (lastSerno + 1)) {
                ...
                la.errs++;
                if (__env.enableDebug) console.debug(la.errs);
                if (la.errs < 3) {
            ...
            switch (packet.action) {
                ...
                case "modify":
                    if (!(packet.arrIndex || packet.hashKey)) {
                        if (__env.enableDebug) console.error("Invalid Packet", packet);
                    } else {
                        la.modify(packet.wireSerno, packet.data, packet.hashKey, packet
                            .arrIndex);
                    }
                    break;
                case "del":
    ...
    $.verto.liveTable = function(verto, context, name, jq, config) {
        ...
        la.onErr = function(obj, args) {
            if (__env.enableDebug) console.error("Error: ", obj, args);
        };
        ...
        la.onChange = function(obj, args) {
            ...
            try {
                switch (args.action) {
                    case "bootObj":
                        if (!args.data) {
                            if (__env.enableDebug) console.error("missing data");
                            return;
                        }
                        dt.fnClearTable();
                ...
                if (__env.enableDebug) console.error("ERROR: " + err);
                iserr++;
            }
    ...
    $.verto.confMan = function(verto, params) {
        ...
        function genMainMod(jq) {
            ...
            var html = "<div id='" + div_id + "'><br>" +
                ...
                (confMan.params.hasVid ? "<button class='ctlbtn' id='" + snapshot_id +
                    "'>PNG Snapshot</button>" : "") +
                "<br><br></div>";
            ...
            if (confMan.params.hasVid) {
                ...
                if (confMan.canvasCount > 1) {
                    html += "<br><br><b>Canvas Controls</b><hr noshade>" +
                        "<button class='ctlbtn' id='" + canvas_in_set_id +
                        "'>Set Input Canvas</button>" +
                        "<button class='ctlbtn' id='" + canvas_in_prev_id +
                        "'>Prev Input Canvas</button>" +
                        "<button class='ctlbtn' id='" + canvas_in_next_id +
                        "'>Next Input Canvas</button>" +

                        "<br>" +

                        "<button class='ctlbtn' id='" + canvas_out_set_id +
                        "'>Set Watching Canvas</button>" +
                        "<button class='ctlbtn' id='" + canvas_out_prev_id +
                        "'>Prev Watching Canvas</button>" +
                        "<button class='ctlbtn' id='" + canvas_out_next_id +
            ...
            verto.subscribe(confMan.params.laData.modChannel, {
                handler: function(v, e) {
                    ...
                    if (e.data["conf-command"] === "list-videoLayouts") {
                        for (var j = 0; j < confMan.canvasCount; j++) {
                            var vlselect_id = "#confman_vl_select_" + j + "_" +
                                confMan.serno;
                            var vlayout_id = "#confman_vid_layout_" + j + "_" +
                                confMan.serno;
                            ...
                            $(vlselect_id).append(new Option("Choose a Layout",
                                "none"));
                            ...
                            if (e.data.responseData) {
                                ...
                                options = rdata.sort(function(a, b) {
                                    var ga = a.substring(0, 6) ==
                                        "group:" ? true : false;
                                    var gb = b.substring(0, 6) ==
                                        "group:" ? true : false;
                                ...
                                for (var i in options) {
                                    $(vlselect_id).append(new Option(options[i],
                                        options[i]));
                                    x++;
                                }
                            }
        ...
        confMan.lt = new $.verto.liveTable(verto, confMan.params.laData.laChannel, confMan.params
            .laData.laName, $(confMan.params.tableID), {
                subParams: {
                    callID: confMan.params.dialog ? confMan.params.dialog.callID : null
                },

                "onChange": function(obj, args) {
                    $(confMan.params.statusID).text("Conference Members: " + " (" +
                        obj.arrayLen() + " Total)");
                    if (confMan.params.onLaChange) {
                        confMan.params.onLaChange(verto, confMan, $.verto.enum.confEvent
                            .laChange, obj, args);
                    }
                },
    ...
    $.verto.dialog = function(direction, verto, params) {
        ...
        if (dialog.direction == $.verto.enum.direction.inbound) {
            ...
            if (!dialog.params.remote_caller_id_name) {
                dialog.params.remote_caller_id_name = "Caller";
            }
            ...
            RTCcallbacks.onMessage = function(rtc, msg) {
                if (__env.enableDebug) console.debug(msg);
            };

            RTCcallbacks.onAnswerSDP = function(rtc, sdp) {
                if (__env.enableDebug) console.error("answer sdp", sdp);
            };
        } else {
        ...
        RTCcallbacks.onICESDP = function(rtc) {
            if (__env.enableDebug) console.log("RECV " + rtc.type + " SDP", rtc.mediaData
                .SDP);

            if (dialog.state == $.verto.enum.state.requesting || dialog.state == $.verto
                .enum.state.answering || dialog.state == $.verto.enum.state.active) {
                location.reload();
                return;
            }
        ...
        RTCcallbacks.onICE = function(rtc) {
            ...
            if (rtc.type == "offer") {
                if (__env.enableDebug) console.log("offer", rtc.mediaData.candidate);
                return;
            }
        };

        RTCcallbacks.onStream = function(rtc, stream) {
            if (__env.enableDebug) console.log("stream started");
        };

        RTCcallbacks.onError = function(e) {
            if (__env.enableDebug) console.error("ERROR:", e);
            dialog.hangup({
    ...
    $.verto.dialog.prototype.setAudioPlaybackDevice = function(sinkId, callback, arg) {
        ...
        if (typeof element.sinkId !== 'undefined') {
            ...
            if (__env.enableDebug) console.info("Dialog: " + dialog.callID +
                " Setting speaker:", element, devname);

            element.setSinkId(sinkId)
                .then(function() {
                    if (__env.enableDebug) console.log("Dialog: " + dialog.callID +
                ...
                .catch(function(error) {
                    ...
                    if (error.name === 'SecurityError') {
                        errorMessage = "Dialog: " + dialog.callID +
                    ...
                    if (__env.enableDebug) console.error(errorMessage);
                });
        } else {
            if (__env.enableDebug) console.warn("Dialog: " + dialog.callID +
    ...
    $.verto.dialog.prototype.setState = function(state) {
        ...
        if (dialog.state == state || !checkStateChange(dialog.state, state)) {
            if (__env.enableDebug) console.error("Dialog " + dialog.callID +
                ": INVALID state change from " + dialog.state.name + " to " + state.name
            );
            dialog.hangup();
            return false;
        }
        ...
        if (__env.enableDebug) console.log("Dialog " + dialog.callID +
            ": state change from " + dialog.state.name + " to " + state.name);
        ...
        switch (dialog.state) {
            ...
            case $.verto.enum.state.active:
                ...
                if (__env.enableDebug) console.info("Using Speaker: ", speaker);
            ...
            case $.verto.enum.state.hangup:
                ...
                if (dialog.lastState.val > $.verto.enum.state.requesting.val && dialog.lastState
                    .val < $.verto.enum.state.hangup.val) {
    ...
    $.verto.dialog.prototype.hangup = function(params) {
        ...
        console.log("HANGUP");
        console.log(params);
        console.log($.verto.enum.state);
        console.log(dialog.state);
        if (dialog.state.val >= $.verto.enum.state.new.val && dialog.state.val < $.verto.enum
            .state.hangup.val) {
            dialog.setState($.verto.enum.state.hangup);
    ...
    $.verto.dialog.prototype.indicateRing = function() {
        ...
        if (dialog.verto.ringer) {
            var playPromise = dialog.verto.ringer.attr("src", dialog.verto.options.ringFile)[
                0].play();
        ...
        if (dialog.state.val >= $.verto.enum.state.early.val) {
            ...
            if (dialog.gotEarly) {
                ...
                dialog.rtc.answer(params.sdp, function() {
                    ...
                    if (__env.enableDebug) console.error(e);
                    dialog.hangup();
                });
                if (__env.enableDebug) console.log("Dialog " + dialog.callID + "ANSWER SDP",
                    params.sdp);
            }
        }
    ...
    $.verto.dialog.prototype.cidString = function(enc) {
        var dialog = this;
        var party = dialog.params.remote_caller_id_name + (enc ? " &lt;" : " <") + dialog.params
            .remote_caller_id_number + (enc ? "&gt;" : ">");
        return party;
    };
    ...
    $.verto.dialog.prototype.handleMedia = function(params) {
        ...
        if (__env.enableDebug) console.log("Dialog " + dialog.callID + "EARLY SDP", params.sdp);
    };
    ...
    var checkDevices = function(runtime) {
        if (__env.enableDebug) console.info("enumerating devices");
        var aud_in = [],
            aud_out = [],
            vid = [];
        ...
        if ((!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) &&
            ...
            MediaStreamTrack.getSources(function(media_sources) {
                ...
                if (__env.enableDebug) console.info("Audio Devices", $.verto.audioInDevices);
                if (__env.enableDebug) console.info("Video Devices", $.verto.videoDevices);
                runtime(true);
            });
        } else {
            ...
            if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
                if (__env.enableDebug) console.log("enumerateDevices() not supported.");
                return;
            }
            ...
            navigator.mediaDevices.enumerateDevices()
                .then(function(devices) {
                    devices.forEach(function(device) {
                        if (__env.enableDebug) console.log(device);

                        if (__env.enableDebug) console.log(device.kind + ": " +
                            device.label +
                            " id = " + device.deviceId);
                    ...
                    if (__env.enableDebug) console.info("Audio IN Devices", $.verto.audioInDevices);
                    if (__env.enableDebug) console.info("Audio Out Devices", $.verto.audioOutDevices);
                    if (__env.enableDebug) console.info("Video Devices", $.verto.videoDevices);
                    runtime(true);

                })
                .catch(function(err) {
                    if (__env.enableDebug) console.log(" Device Enumeration ERROR: " +
