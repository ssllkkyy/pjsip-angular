...
(function($) {
    ...
    function onStreamSuccess(self, stream) {
        if (__env.enableDebug) console.log("Stream Success");
    ...
    function onRemoteStream(self, stream) {
        if (self.options.useVideo) {
            ...
            if (__env.enableDebug) console.log("abcabcabc");
            document.getElementById('video-container').style.display = 'block';
        }

        var element = self.options.useAudio;
        if (__env.enableDebug) console.log("REMOTE STREAM", stream, element);
        ...
        if (typeof element.srcObject !== 'undefined') {
            ...
            if (__env.enableDebug) console.error('Error attaching stream to element.');
        }
        3
    ...
    function onOfferSDP(self, sdp) {
        self.mediaData.SDP = self.stereoHack(sdp.sdp);
        if (__env.enableDebug) console.log("Offer SDP");
    ...
    $.FSRTC.prototype.stopPeer = function() {
        if (self.peer) {
            if (__env.enableDebug) console.log("stopping peer");
            self.peer.stop();
        }
    }
        ...
        if (self.localStream) {
            if (typeof self.localStream.stop == 'function') {
                ...
                if (self.localStream.active) {
                    ...
                    if (__env.enableDebug) console.error(tracks);
                    tracks.forEach(function(track, index) {
                        if (__env.enableDebug) console.log(track);
                        track.stop();
                    })
                }
            }
    ...
    $.FSRTC.prototype.createAnswer = function(params) {
        ...
        if (__env.enableDebug) console.debug("inbound sdp: ", params.sdp);
        ...
        if (__env.enableDebug) console.log("Audio constraints", mediaParams.audio);
        if (__env.enableDebug) console.log("Video constraints", mediaParams.video);
        ...
        if (self.options.useVideo && self.options.localVideo) {
            getUserMedia({
                ...
                onsuccess: function(e) {
                    ...
                    if (__env.enableDebug) console.log("local video ready");
                },
                onerror: function(e) {
                    if (__env.enableDebug) console.error("local video error!");
                }
            });
        }
    ...
    function getMediaParams(obj) {
        ...
        if (obj.options.useMic && obj.options.useMic === "none") {
            if (__env.enableDebug) console.log("Microphone Disabled");
            audio = false;
            ...
            if (__env.enableDebug) console.error("SCREEN SHARE");
            audio = false;
        } else {
            audio = {
                mandatory: {},
        ...
        if (useVideo && obj.options.useCamera && obj.options.useCamera !== "none") {
            ...
            if (__env.enableDebug) console.log("Camera Disabled");
            video = false;
            useVideo = false;
        }
    ...
    function RTCPeerConnection(options) {
        ...
        if (options.iceServers) {
            ...
            if (tmp && !(typeof(tmp) == "object" && tmp.constructor === Array)) {
                if (__env.enableDebug) console.warn(
        ...
        function ice_handler() {
            ...
            if (options.type == "offer") {
                if ((!moz || (!options.sentICESDP && peer.localDescription.sdp.match(
                        /a=candidate/)) && !x && options.onICESDP)) {
        ...
        peer.onaddstream = function(event) {
            ...
            remoteMediaStream.onended = function() {
                if (options.onRemoteStreamEnded) options.onRemoteStreamEnded(
                    remoteMediaStream);
            };
        ...
        function createOffer() {
            ...
            peer.createOffer(function(sessionDescription) {
                    ...
                    if (moz && options.onICESDP && sessionDescription.sdp.match(
                            /a=candidate/)) {
                        options.onICESDP(sessionDescription);
        ...
        function createAnswer() {
            ...
            peer.setRemoteDescription(new SessionDescription(options.offerSDP), onSdpSuccess,
                onSdpError);
            peer.createAnswer(function(sessionDescription) {
                    sessionDescription.sdp = serializeSdp(sessionDescription.sdp);
                    peer.setLocalDescription(sessionDescription);
        ...
        function getInteropSDP(sdp) {
            ...
            if (options.onAnswerSDP) sdp = sdp.replace(
            ...
            sdp = sdp.indexOf('a=crypto') == -1 ? sdp.replace(/c=IN/g,
                'a=crypto:1 AES_CM_128_HMAC_SHA1_80 inline:' + inline + 'c=IN') : sdp;
        ...
        function setChannelEvents() {
            ...
            channel.onclose = function(event) {
                ...
                if (__env.enableDebug) console.warn('WebRTC DataChannel closed', event);
            };
            channel.onerror = function(event) {
                if (options.onChannelError) options.onChannelError(event);

                if (__env.enableDebug) console.error('WebRTC DataChannel error', event);
            };
        }
        ...
        function onSdpError(e) {
            ...
            if (__env.enableDebug) console.error('sdp error:', e);
        }

        return {
            addAnswerSDP: function(sdp, cbSuccess, cbError) {

                peer.setRemoteDescription(new SessionDescription(sdp), cbSuccess ?
                    cbSuccess : onSdpSuccess, cbError ? cbError : onSdpError);
            },
    ...
    function getUserMedia(options) {
        ...
        function streaming(stream) {
            ...
            if (options.localVideo) {
                options.localVideo[moz ? 'mozSrcObject' : 'src'] = moz ? stream : window.webkitURL
    ...
    $.FSRTC.bestResSupported = function() {
        ...
        for (var i in $.FSRTC.validRes) {
            if ($.FSRTC.validRes[i][0] > width && $.FSRTC.validRes[i][1] > height) {
                width = $.FSRTC.validRes[i][0];
                height = $.FSRTC.validRes[i][1];
            }
        }
    ...
    var checkRes = function(cam, func) {
        ...
        width = resList[resI][0];
        height = resList[resI][1];
        resI++;
        ...
        getUserMedia({
            ...
            onsuccess: function(e) {
                ...
                if (__env.enableDebug) console.info(width + "x" + height +
                    " supported.");
                $.FSRTC.validRes.push([width, height]);
                checkRes(cam, func);
            },
            onerror: function(e) {
                if (__env.enableDebug) console.error(width + "x" + height +
                    " not supported.");
                checkRes(cam, func);
            }
        });
    ...
    $.FSRTC.getValidRes = function(cam, func) {
        ...
        if (cached) {
            ...
            if (cache) {
                ...
                if (__env.enableDebug) console.log("CACHED RES FOR CAM " + cam, cache);
            } else {
                if (__env.enableDebug) console.error("INVALID CACHE");
            }
    ...
    $.FSRTC.checkPerms = function(runtime, check_audio, check_video) {
        getUserMedia({
            ...
            onsuccess: function(e) {
                ...
                if (__env.enableDebug) console.info("media perm init complete");
                if (runtime) {
            ...
            onerror: function(e) {
                if (check_video && check_audio) {
                    if (__env.enableDebug) console.error(
                        "error, retesting with audio params only");
                    return $.FSRTC.checkPerms(runtime, check_audio, false);
                }

                if (__env.enableDebug) console.error("media perm init error");
