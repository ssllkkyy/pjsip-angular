    ...
    $.JsonRpcClient.prototype.call = function(method, params, success_cb, error_cb) {
        ...
        if (!success_cb) {
            success_cb = function(e) {
                if (__env.enableDebug) console.log("Success: ", e);
            };
        }

        if (!error_cb) {
            error_cb = function(e) {
                if (__env.enableDebug) console.log("Error: ", e);
            };
        }
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                try {
                    ...
                    if ('console' in window)
                        if (__env.enableDebug) console.log(response); // Ugly, but it should work.
        ...
        if (!self.socketReady()) {
            ...
            if (self._ws_socket) {
                ...
                self._ws_socket.onclose = function(w) {
                    ...
                    if (__env.enableDebug) console.error("Websocket Lost " + self.ws_cnt +
                        " sleep: " + self.ws_sleep + "msec");
    ...
    $.JsonRpcClient.prototype._wsOnMessage = function(event) {
        ...
        if (event.data[0] == "#" && event.data[1] == "S" && event.data[2] == "P") {
            if (event.data[3] == "U") {
                ...
                var up_kps = (((this.speedBytes * 8) / (this.up_dur / 1000)) / 1024).toFixed(
                    0);
                var down_kps = (((this.speedBytes * 8) / (this.down_dur / 1000)) / 1024).toFixed(
                    0);

                if (__env.enableDebug) console.info("Speed Test: Up: " + up_kps + " Down: " +
                    down_kps);
                this.speedCB(event, {
                    upDur: this.up_dur,
        ...
        try {
            ...
            if (typeof response === 'object' &&
                ...
                if ('result' in response && this._ws_callbacks[response.id]) {
                    ...
                    */
                    ...
                    if (!self.authing && response.error.code == -32000 && self.options.login &&
                        self.options.passwd) {
                        self.authing = true;
                        ...
                        this.call("login", {
                            ...
                            this._ws_callbacks[response.id].request_obj.method ==
                            "login" ?
                            function(e) {
                                self.authing = false;
                                if (__env.enableDebug) console.log("logged in");
                            ...
                            function(e) {
                                if (__env.enableDebug) console.log(
                                    "error logging in, request id:", response.id);
                                delete self._ws_callbacks[response.id];
                                error_cb(response.error, this);
                                if (self.options.onWSLogin) {
            ...
            if (__env.enableDebug) console.log("ERROR: " + err);
            return;
        }
