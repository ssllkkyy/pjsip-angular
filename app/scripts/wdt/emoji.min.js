    ...
    var f = this,
        ...
        a = function() {
            ...
            return f.img_set = "apple", f.img_sets = {
                ...
                }, f.use_css_imgs = !1, f.colons_mode = !1, f.text_mode = !1, f.include_title = !
                1, f.include_text = !1, f.allow_native = !0, f.use_sheet = !1, f.avoid_ms_emoji = !
                0, f.allow_caps = !1, f.img_suffix = "", f.inits = {}, f.map = {}, f.init_env(),
                f
        };
    a.prototype.noConflict = function() {
            return f.EmojiConvertor = e, a
        }, a.prototype.replace_emoticons = function(f) {
            var e = this,
                a = e.replace_emoticons_with_colons(f);
            return e.replace_colons(a)
        }, a.prototype.replace_emoticons_with_colons = function(f) {
            var e = this;
            e.init_emoticons();
            var a = 0,
                o = [],
                t = f.replace(e.rx_emoticons, function(t, i, n, l) {
                    var r = a;
                    a = l + t.length;
                    var c = -1 !== n.indexOf("("),
                        _ = -1 !== n.indexOf(")");
                    if ((c || _) && -1 == o.indexOf(n) && o.push(n), _ && !c) {
                        var s = f.substring(r, l);
                        if (-1 !== s.indexOf("(") && -1 === s.indexOf(")")) return t
                    }
                    if ("\n8)" === t) {
                        var d = f.substring(0, l);
                        if (/\n?(6\)|7\))/.test(d)) return t
                    }
                    var g = e.data[e.map.emoticons[n]][3][0];
                    return g ? i + ":" + g + ":" : t
                });
            if (o.length) {
                var i = o.map(e.escape_rx),
                    n = new RegExp("(\\(.+)(" + i.join("|") + ")(.+\\))", "g");
                t = t.replace(n, function(f, a, o, t) {
                    var i = e.data[e.map.emoticons[o]][3][0];
                    return i ? a + ":" + i + ":" + t : f
                })
            }
            return t
        }, a.prototype.replace_colons = function(f) {
            var e = this;
            return e.init_colons(), f.replace(e.rx_colons, function(f) {
                var a = f.substr(1, f.length - 2);
                if (e.allow_caps && (a = a.toLowerCase()), a.indexOf("::skin-tone-") >
                    -1) {
                    var o = a.substr(-1, 1),
                        t = "skin-tone-" + o,
                        i = e.map.colons[t];
                    a = a.substr(0, a.length - 13);
                    var n = e.map.colons[a];
                    return n ? e.replacement(n, a, ":", {
                        idx: i,
                        actual: t,
                        wrapper: ":"
                    }) : ":" + a + ":" + e.replacement(i, t, ":")
                ...
                var n = e.map.colons[a];
                return n ? e.replacement(n, a, ":") : f
            })
        }, a.prototype.replace_unified = function(f) {
            var e = this;
            return e.init_unified(), f.replace(e.rx_unified, function(f, a, o) {
                var t = e.map.unified[a];
                if (!t) return f;
                var i = null;
                return "🏻" == o && (i = "1f3fb"), "🏼" == o && (i = "1f3fc"), "🏽" ==
                    o && (i = "1f3fd"), "🏾" == o && (i = "1f3fe"), "🏿" == o && (i =
                        "1f3ff"), i ? e.replacement(t, null, null, {
                        idx: i,
                        actual: o,
                        wrapper: ""
                    }) : e.replacement(t)
            })
        }, a.prototype.addAliases = function(f) {
            var e = this;
            e.init_colons();
            for (var a in f) e.map.colons[a] = f[a]
        }, a.prototype.removeAliases = function(f) {
            for (var e = this, a = 0; a < f.length; a++) {
                var o = f[a];
                delete e.map.colons[o];
                f: for (var t in e.data)
                    for (var i = 0; i < e.data[t][3].length; i++)
                        if (o == e.data[t][3][i]) {
                            e.map.colons[o] = t;
                            break f
                        }
            }
        }, a.prototype.replacement = function(f, e, a, o) {
            var t = this,
                i = f,
                n = "",
                l = 0;
            "object" == typeof o && (n = t.replacement(o.idx, o.actual, o.wrapper), l = f + "-" +
                o.idx);
            var r = t.img_set;
            if (t.use_sheet && t.supports_css || t.data[f][6] & t.img_sets[t.img_set].mask || (
                    r = "apple"), a = a || "", t.colons_mode) return ":" + t.data[f][3][0] +
                ":" + n;
            var c = e ? a + e + a : t.data[f][8] || a + t.data[f][3][0] + a;
            if (t.text_mode) return c + n;
            if (t.init_env(), "unified" == t.replace_mode && t.allow_native && t.data[f][0][0])
                return t.data[f][0][0] + n;
            if ("softbank" == t.replace_mode && t.allow_native && t.data[f][1]) return t.data[f]
                [1] + n;
            if ("google" == t.replace_mode && t.allow_native && t.data[f][2]) return t.data[f][
                2
            ] + n;
            var _ = t.data[f][7] || t.img_sets[r].path + f + ".png" + t.img_suffix,
                s = t.include_title ? ' title="' + (e || t.data[f][3][0]) + '"' : "",
                d = t.include_text ? a + (e || t.data[f][3][0]) + a : "",
                g = t.data[f][4],
                b = t.data[f][5];
            if (l && t.variations_data[l] && t.variations_data[l][2] && !t.data[f][7] && t.variations_data[
                    l][2] & t.img_sets[t.img_set].mask && (_ = t.img_sets[t.img_set].path + l +
                    ".png", g = t.variations_data[l][0], b = t.variations_data[l][1], n = "", i =
                    l, t.include_text && o && o.actual && o.wrapper && (d += o.wrapper + o.actual +
                        o.wrapper)), t.supports_css) {
                if (t.use_sheet && null != g && null != b) {
                    var m = 100 / (t.sheet_size - 1),
                        p = "background: url(" + t.img_sets[r].sheet + ");background-position:" +
                        m * g + "% " + m * b + "%;background-size:" + t.sheet_size + "00%";
                    return '<span class="emoji-outer emoji-sizer"><span class="emoji-inner" style="' +
                        p + '"' + s + ' data-codepoints="' + i + '">' + d + "</span></span>" +
                        n
                }
                return t.use_css_imgs ? '<span class="emoji emoji-' + f + '"' + s +
                    ' data-codepoints="' + i + '">' + d + "</span>" + n :
                    '<span class="emoji emoji-sizer" style="background-image:url(' + _ + ')"' +
                    s + ' data-codepoints="' + i + '">' + d + "</span>" + n
            }
            return '<img src="' + _ + '" class="emoji" data-codepoints="' + i + '" ' + s + "/>" +
                n
        }, a.prototype.init_emoticons = function() {
            var f = this;
            if (!f.inits.emoticons) {
                ...
                for (var a in f.emoticons_data) {
                    var o = a.replace(/\&/g, "&amp;").replace(/\</g, "&lt;").replace(/\>/g,
                        "&gt;");
                    f.map.colons[f.emoticons_data[a]] && (f.map.emoticons[o] = f.map.colons[f.emoticons_data[
                        a]], e.push(f.escape_rx(o)))
                }
                f.rx_emoticons = new RegExp("(^|\\s)(" + e.join("|") + ")(?=$|[\\s|\\?\\.,!])",
                    "g")
            }
        }, a.prototype.init_colons = function() {
            var f = this;
            if (!f.inits.colons) {
                f.inits.colons = 1, f.rx_colons = new RegExp(
                    ":[a-zA-Z0-9-_+]+:(:skin-tone-[2-6]:)?", "g"), f.map.colons = {};
                for (var e in f.data)
                    for (var a = 0; a < f.data[e][3].length; a++) f.map.colons[f.data[e][3][a]] =
                        e
            }
        }, a.prototype.init_unified = function() {
            var f = this;
            if (!f.inits.unified) {
                f.inits.unified = 1;
                var e = [];
                f.map.unified = {};
                for (var a in f.data)
                    for (var o = 0; o < f.data[a][0].length; o++) e.push(f.data[a][0][o].replace(
                        "*", "\\*")), f.map.unified[f.data[a][0][o]] = a;
                e = e.sort(function(f, e) {
                    return e.length - f.length
                }), f.rx_unified = new RegExp("(" + e.join("|") + ")(�[�-�])?", "g")
            }
        }, a.prototype.init_env = function() {
            var f = this;
            if (!f.inits.env) {
                if (f.inits.env = 1, f.replace_mode = "img", f.supports_css = !1, "undefined" !=
                    typeof navigator) {
                    var e = navigator.userAgent;
                    if (window.getComputedStyle) try {
                        var a = window.getComputedStyle(document.body);
                        (a["background-size"] || a.backgroundSize) && (f.supports_css = !0)
                    } catch (o) {
                        e.match(/Firefox/i) && (f.supports_css = !0)
                    }
                    if (e.match(/(iPhone|iPod|iPad|iPhone\s+Simulator)/i)) {
                        if (e.match(/OS\s+[12345]/i)) return void(f.replace_mode = "softbank");
                        if (e.match(/OS\s+[6789]/i)) return void(f.replace_mode = "unified")
                    }
                    if (e.match(/Mac OS X 10[._ ](?:[789]|1\d)/i)) return void(f.replace_mode =
                        "unified");
                    if (!f.avoid_ms_emoji && (e.match(/Windows NT 6.[1-9]/i) || e.match(
                            /Windows NT 10.[0-9]/i)) && !e.match(/Chrome/i) && !e.match(
                            /MSIE 8/i)) return void(f.replace_mode = "unified")
                }
                f.supports_css && (f.replace_mode = "css")
            }
        }, a.prototype.escape_rx = function(f) {
            return f.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&")
        }, a.prototype.sheet_size = 41, a.prototype.data = {
            "00a9": [
            ...
            "23ee": [
                ["⏮"], "", "", ["black_left_pointing_double_triangle_with_vertical_bar"], 0,
                23, 15, 0
            ],
            "23ef": [
            ...
            2757: [
                ["❗️", "❗"], "", "󾬄", ["exclamation", "heavy_exclamation_mark"], 4, 8, 15,
                0
            ],
            2763: [
            ...
            "1f326": [
                ["🌦"], "", "", ["partly_sunny_rain", "sun_behind_rain_cloud"], 6, 16, 15,
                0
            ],
            "1f327": [
            ...
            exports = module.exports = a), exports.EmojiConvertor = a) : "function" == typeof define &&
